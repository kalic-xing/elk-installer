...
#
# Usage:
#   sudo ./install-agent.sh -i <Windows IP> -u <username> -p <password> -o <operating system>
#
# Options:
#   -i : IP address of the Windows machine
...
parse_args() {
    ...
    if [ -z "$windows_ip" ] || [ -z "$operating_system" ] || [ -z "$username" ] || [ -z "$password" ]; then
...
EOF
    ...
    info "Uploading the Elastic Agent zip file to $windows_ip..."
...
install_on_linux() {
    info "Linux installation not yet implemented."
    exit 0
}
